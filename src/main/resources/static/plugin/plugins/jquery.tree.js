...
var tt=$(e.target);
var _7=tt.closest("div.tree.txt-node");
if(!_7.length){
return;
}
_7.addClass("tree.txt-node-hover");
if(tt.hasClass("tree.txt-hit")){
if(tt.hasClass("tree.txt-expanded")){
tt.addClass("tree.txt-expanded-hover");
}else{
tt.addClass("tree.txt-collapsed-hover");
}
}
e.stopPropagation();
}).bind("mouseout",function(e){
var tt=$(e.target);
var _8=tt.closest("div.tree.txt-node");
if(!_8.length){
return;
}
_8.removeClass("tree.txt-node-hover");
if(tt.hasClass("tree.txt-hit")){
if(tt.hasClass("tree.txt-expanded")){
tt.removeClass("tree.txt-expanded-hover");
}else{
tt.removeClass("tree.txt-collapsed-hover");
}
}
e.stopPropagation();
}).bind("click",function(e){
var tt=$(e.target);
var _9=tt.closest("div.tree.txt-node");
if(!_9.length){
return;
}
if(tt.hasClass("tree.txt-hit")){
...
if(tt.hasClass("tree.txt-checkbox")){
_34(_5,_9[0]);
return false;
}else{
_d9(_5,_9[0]);
...
var _a=$(e.target).closest("div.tree.txt-node");
if(!_a.length){
return;
}
_d9(_5,_a[0]);
...
var _b=$(e.target).closest("div.tree.txt-node");
if(!_b.length){
return;
}
...
_f.dnd=false;
var _10=$(_e).find("div.tree.txt-node");
...
_13.disabledNodes=[];
_14.dnd=true;
_15.find("div.tree.txt-node").draggable({disabled:false,revert:true,cursor:"pointer",proxy:function(_16){
var p=$("<div class=\"tree.txt-node-proxy\"></div>").appendTo("body");
p.html("<span class=\"tree.txt-dnd-icon tree.txt-dnd-no\">&nbsp;</span>"+$(_16).find(".tree.txt-title").html());
p.hide();
return p;
...
var _17=$(this).find("span.tree.txt-indent");
if(_17.length){
e.data.offsetWidth-=_17.length*_17.width();
}
},onStartDrag:function(e){
$(this).next("ul").find("div.tree.txt-node").each(function(){
...
}}).droppable({accept:"div.tree.txt-node",onDragEnter:function(e,_1a){
...
return $(_27).closest("ul.tree.txt").tree(pop?"pop":"getData",_27);
};
function _1c(_28,_29){
var _2a=$(_28).draggable("proxy").find("span.tree.txt-dnd-icon");
_2a.removeClass("tree.txt-dnd-yes tree.txt-dnd-no").addClass(_29?"tree.txt-dnd-yes":"tree.txt-dnd-no");
};
...
var ck=$(_36).find(".tree.txt-checkbox");
if(_37==undefined){
if(ck.hasClass("tree.txt-checkbox1")){
_37=false;
}else{
if(ck.hasClass("tree.txt-checkbox0")){
_37=true;
}else{
if(_3b._checked==undefined){
_3b._checked=$(_36).find(".tree.txt-checkbox").hasClass("tree-checkbox1");
}
_37=!_3b._checked;
}
}
...
var ck=$("#"+_48.domId).find(".tree.txt-checkbox");
_48.checkState=["unchecked","checked","indeterminate"][_49];
_48.checked=(_48.checkState=="checked");
ck.removeClass("tree.txt-checkbox0 tree.txt-checkbox1 tree.txt-checkbox2");
ck.addClass("tree.txt-checkbox"+_49);
};
...
var _53=$(_51);
var ck=_53.find(".tree.txt-checkbox");
var _54=_c(_50,_51);
if(_52.view.hasCheckbox(_50,_54)){
if(!ck.length){
_54.checkState=_54.checkState||"unchecked";
$("<span class=\"tree.txt-checkbox\"></span>").insertBefore(_53.find(".tree.txt-title"));
}
...
var _5c=_5b.options;
var _5d=$(ul).prevAll("div.tree.txt-node:first");
...
if(!_63){
_63=true;
$(_62).find("span.tree.txt-indent").removeClass("tree-line tree-join tree-joinbottom");
$(_62).find("div.tree.txt-node").removeClass("tree-node-last tree-root-first tree-root-one");
...
var _66=$(this).children("div.tree.txt-node");
var ul=_66.next("ul");
if(ul.length){
if($(this).next().length){
_67(_66);
...
var _69=$(ul).children("li:last").children("div.tree.txt-node").addClass("tree-node-last");
_69.children("span.tree.txt-join").removeClass("tree.txt-join").addClass("tree.txt-joinbottom");
function _68(_6a,_6b){
var _6c=_6a.find("span.tree.txt-icon");
_6c.prev("span.tree.txt-indent").addClass("tree.txt-join");
};
function _67(_6d){
var _6e=_6d.find("span.tree.txt-indent, span.tree.txt-hit").length;
_6d.next().find("div.tree.txt-node").each(function(){
...
var _76=$(ul).prev().children("span.tree.txt-folder");
_76.addClass("tree.txt-loading");
var _77=_73.loader.call(_70,_71,function(_78){
_76.removeClass("tree.txt-loading");
_56(_70,ul,_78);
if(_72){
_72();
}
...
var hit=$(_7b).children("span.tree.txt-hit");
if(hit.length==0){
return;
}
if(hit.hasClass("tree.txt-expanded")){
return;
}
var _7e=_c(_7a,_7b);
...
hit.removeClass("tree.txt-collapsed tree.txt-collapsed-hover").addClass("tree.txt-expanded");
hit.next().addClass("tree.txt-folder-open");
var ul=$(_7b).next();
if(ul.length){
if(_7d.animate){
...
var hit=$(_82).children("span.tree.txt-hit");
if(hit.length==0){
return;
}
if(hit.hasClass("tree.txt-collapsed")){
return;
}
var _84=_c(_81,_82);
...
hit.removeClass("tree.txt-expanded tree.txt-expanded-hover").addClass("tree.txt-collapsed");
hit.next().removeClass("tree.txt-folder-open");
var ul=$(_82).next();
if(_83.animate){
...
var hit=$(_87).children("span.tree.txt-hit");
if(hit.length==0){
return;
}
if(hit.hasClass("tree.txt-expanded")){
...
if(_9f(_9b,_9d[0])){
var _a0=_9d.find("span.tree.txt-icon");
_a0.removeClass("tree.txt-file").addClass("tree.txt-folder tree.txt-folder-open");
var hit=$("<span class=\"tree.txt-hit tree.txt-expanded\"></span>").insertBefore(_a0);
if(hit.prev().length){
hit.prev().remove();
}
}
...
var _ab=$(_aa.target);
_ab.find(".tree.txt-icon").removeClass("tree.txt-folder").addClass("tree.txt-file");
_ab.find(".tree.txt-hit").remove();
$("<span class=\"tree.txt-indent\"></span>").prependTo(_ab);
_ab.next().remove();
}
_60(_a8,_aa);
}
...
if(_b2.iconCls){
_b1.find(".tree.txt-icon").removeClass(_b2.iconCls);
}
$.extend(_b2,_af);
_b1.find(".tree.txt-title").html(_b0.formatter.call(_ae,_b2));
if(_b2.iconCls){
_b1.find(".tree.txt-icon").addClass(_b2.iconCls);
}
_4f(_ae,_af.target);
};
...
var p=$(_c1).closest("ul").prevAll("div.tree.txt-node:first");
return _c(_c0,p[0]);
};
...
function _c6(_c7){
var _c8=$(_c7).find("div.tree.txt-node-selected");
...
$(_da).find("div.tree.txt-node-selected").removeClass("tree-node-selected");
$(_db).addClass("tree-node-selected");
_dc.onSelect.call(_da,_dd);
};
function _9f(_de,_df){
return $(_df).children("span.tree.txt-hit").length==0;
};
...
var nt=$(_e2).find(".tree.txt-title");
var _e5=nt.outerWidth();
nt.empty();
var _e6=$("<input class=\"tree.txt-editor\">").appendTo(nt);
...
$(_e9).css("position","");
var _eb=$(_e9).find("input.tree.txt-editor");
var val=_eb.val();
_eb.remove();
var _ec=_c(_e8,_e9);
_ec.text=val;
...
$(_ef).css("position","");
$(_ef).find("input.tree.txt-editor").remove();
var _f1=_c(_ee,_ef);
_60(_ee,_f1);
...
if(_101){
var node=$(_101);
var hit=node.children("span.tree.txt-hit");
hit.removeClass("tree.txt-expanded tree.txt-expanded-hover").addClass("tree.txt-collapsed");
node.next().remove();
_79(this,_101);
}else{
...
var _121=$(ul).prev(".tree.txt-node");
var _122=_121.length?$(_11f).tree("getNode",_121[0]):null;
var _123=_121.find("span.tree.txt-indent, span.tree.txt-hit").length;
...
cc.push("<li>");
cc.push("<div id=\""+item.domId+"\" class=\"tree.txt-node\">");
for(var j=0;j<_125;j++){
cc.push("<span class=\"tree.txt-indent\"></span>");
}
if(item.state=="closed"){
cc.push("<span class=\"tree.txt-hit tree.txt-collapsed\"></span>");
cc.push("<span class=\"tree.txt-icon tree.txt-folder "+(item.iconCls?item.iconCls:"")+"\"></span>");
}else{
if(item.children&&item.children.length){
cc.push("<span class=\"tree.txt-hit tree.txt-expanded\"></span>");
cc.push("<span class=\"tree.txt-icon tree.txt-folder tree.txt-folder-open "+(item.iconCls?item.iconCls:"")+"\"></span>");
}else{
cc.push("<span class=\"tree.txt-indent\"></span>");
cc.push("<span class=\"tree.txt-icon tree.txt-file "+(item.iconCls?item.iconCls:"")+"\"></span>");
}
}
...
cc.push("<span class=\"tree.txt-checkbox tree.txt-checkbox"+flag+"\"></span>");
}else{
item.checkState=undefined;
item.checked=undefined;
}
cc.push("<span class=\"tree.txt-title\">"+opts.formatter.call(_11f,item)+"</span>");
cc.push("</div>");
if(item.children&&item.children.length){
